...
{
    public partial class JobHistoryChart2 : Form
    {
        private readonly string connectionString;
        private readonly ScottPlot.Styles.IStyle plotStyle = new MyPlotStyle();

        public JobHistoryChart2(string connectionString)
        {
            InitializeComponent();
        ...
        public void DrawChart(DateTime searchDate1, DateTime searchDate2, JobHistoryChartConfigFilter filteredItems)
        {
            using (var con = new SqlConnection(connectionString))
            {
                ...
                plt.Style(plotStyle);
                plt.Title("출발지별 반송량&평균반송시간");

                //if (filteredItems?.StartPos?.Count > 0)
                {
                    ...
                    params1.Add("lineNames", string.Join(",", filteredItems.StartPos));

                    var result = con.Query(StoredProcedureNames.GetJobHistoryAggr2_JobHistoryTransportValueColumnAdd
                    ...
                    if (result.Count() > 0)
                    {
                        ...
                        string[] labels = result.Select(x => (string)x.출발지).ToArray();
